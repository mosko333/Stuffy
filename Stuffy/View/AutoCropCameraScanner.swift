...
class RectangleFeature: CIFeature {
    ...
    class func setValue(topLeft:CGPoint, topRight:CGPoint, bottomLeft:CGPoint, bottomRight:CGPoint) -> RectangleFeature {
        let obj:RectangleFeature = RectangleFeature()
        obj.topLeft = topLeft
...
class AutoCropCameraScanner:UIView, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    var borderDetectLastRectangleFeature: RectangleFeature?
    ...
    func setCameraViewType(_ cameraViewType: CameraViewType) {
    ...
    func captureOutput(_ captureOutput: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        if cameraViewType != CameraViewType.normal {
    ...
    func biggestRectangle(inRectangles rectangles: [Any]) -> RectangleFeature? {
    ...
    func start() {
        ...
        borderDetectTimeKeeper = Timer.scheduledTimer(timeInterval: 0.5, target: self, selector: #selector(AutoCropCameraScanner.enableBorderDetectFrame), userInfo: nil, repeats: true)
        hideGLKView(false)
    }
    ...
    func correctPerspective(for image: CIImage, withFeatures rectangleFeature: RectangleFeature) -> CIImage {
